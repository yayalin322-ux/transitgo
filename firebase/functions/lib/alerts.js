...
import { tdxConfigured, traAlerts, thsrAlerts } from "./tdx.js";
import { createAnnouncement, getAlertState, setAlertState } from "./db.js";
import { pushAnnouncement } from "./push.js";

const sig = (items) =>
  createHash("sha1").update(items.map((i) => `${i.title}|${i.description}`).join("\n")).digest("hex");
...
async function checkSource({ source, category, label, fetcher }) {
  ...
  if (result.abnormal) {
    const first = result.items.find((i) => i.title && !/正常|normal/i.test(i.title));
    const ann = await createAnnouncement({
      category, severity: "warning", title: `${label}營運異常`,
    ...
    const ann = await createAnnouncement({
      category, severity: "info", title: `${label}已恢復正常`, body: "", source,
...
/** Runs one pass over TRA + THSR. Called from the scheduled function in index.js. */
export async function runAlertPoll() {
  if (!tdxConfigured()) {
    console.log("[alerts] TDX not configured — skipping");
    return;
  }
