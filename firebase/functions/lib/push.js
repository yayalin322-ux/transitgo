...
try {
  ...
  const { APNS_KEY_PATH, APNS_KEY_CONTENT, APNS_KEY_ID, APNS_TEAM_ID, APNS_BUNDLE_ID, APNS_PRODUCTION } = process.env;

  // Cloud Functions has no persistent disk to hold a .p8 file — prefer APNS_KEY_CONTENT
  // (the .p8's contents, PEM text) set via `firebase functions:secrets:set APNS_KEY_CONTENT`.
  // APNS_KEY_PATH still works for local/emulator use.
 ...
 * @param {{id:string, category:string, severity:string, title:string, body:string}} ann
 */
export async function pushAnnouncement(ann) {
  ...
  const payload = {
    title: ann.title,
    body: ann.body || " ",
    data: { type: "announcement", id: String(ann.id), category: ann.category, severity: ann.severity },
  };

  if (dryRun || !provider) {
    console.log("[push:dry] → %d devices | %s | %s / %s", tokens.length, ann.category, ann.title, ann.body);
