...
function graphKey(artifactId) {
  return `${PREFIX}/${artifactId}.graph`;
}
 ...
 * saveGraphToDisk) via a read stream — never reads it into one Buffer first. Also
...
export async function uploadArtifact(client, { bucket, localPath, artifactId, meta }) {
  ...
  const metaBody = JSON.stringify({ ...meta, artifactId, sizeBytes: size });
  await client.send(new PutObjectCommand({
    Bucket: bucket,
  ...
  logMemory("storage_upload_complete", { artifactId, sizeMB: Math.round(size / 1024 / 1024) });
  return { size };
}
...
/** Streams the artifact body straight to a local temp file — never accumulates chunks
 * into an in-memory array/Buffer, which is exactly the kind of second-giant-buffer
 * duplication PR #2's streaming persist was written to eliminate; reintroducing it here
 * on the download side would be the same mistake in a different place. */
