...
// tamper with the bytes actually stored in the fake bucket, then download+load again.
const corrupted = Buffer.from(readFileSync(localBuildPath));
corrupted[Math.floor(corrupted.length / 2)] ^= 0xff; // flip a byte in the middle of the payload
client.objects.set(`routing-graph/production/${artifactId}.graph`, corrupted);
...
const loadedCorrupt = loadGraphFromDisk(corruptDownloadPath);
check("A corrupted artifact (flipped byte) is rejected by checksum validation, not silently loaded", loadedCorrupt === null);
